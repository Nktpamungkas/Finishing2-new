...
{     
	...
	if(lprn=="Adm Finishing"){window.location.href="?p=home2";}
...
<form id="form1" name="form1" method="post" action="index.php?p=reports" >
      <table width="470" border="0">
        <tr>
          <td colspan="3"><div align="center"><strong>LAPORAN PRODUKSI FINISHING</strong></div> 
         </div>
          <?php 
        ...
        <tr valign="middle">
          ...
          <td><select name="jnsmesin" id="jnsmesin" onChange="window.location='?p=home&jns='+this.value" required>
            ...
            <option value="steamer" <?php if($_GET['jns']=="steamer"){echo"SELECTED";}?>>Steamer</option>
            <option value="belah" <?php if($_GET['jns']=="belah"){echo"SELECTED";}?>>Belah</option>
            <option value="compact" <?php if($_GET['jns']=="compact"){echo"SELECTED";}?>>Compact</option>
            <option value="lipat" <?php if($_GET['jns']=="lipat"){echo"SELECTED";}?>>Lipat</option>
            <option value="oven" <?php if($_GET['jns']=="oven"){echo"SELECTED";}?>>Oven</option>
			      <option value="stenter" <?php if($_GET['jns']=="stenter"){echo"SELECTED";}?>>Stenter</option>
          </select></td>
        </tr>
        <tr valign="middle">
        ...
        <tr>
          ...
          <td><select name="nama_mesin" id="nama_mesin" onchange="myFunction();">
            <option value="">Pilih</option>  
            <?php $qry1=mysqli_query($con,"SELECT no_mesin FROM tbl_no_mesin WHERE jenis LIKE '$_GET[jns]%' ORDER BY no_mesin ASC"); 
