...
session_start();
include("../../koneksi.php");
	if($_GET['mc']=="stenter"){$mc="Stenter";}else if($_GET['mc']=="compact"){$mc="Compact";}else if($_GET['mc']=="oven"){$mc="Oven";}
...
<form id="form1" name="form1" method="post" action="" >
 <fieldset>
  <legend>Data Produksi Harian <?php echo $mc;?></legend>
  <table width="100%" border="0">
    ...
    <tr>
      ...
      <td colspan="2"><select name="nama_mesin" id="nama_mesin" onchange="myFunction();" required="required">
        <option value="">Pilih</option>
        <?php $qry1=mysqli_query($con,"SELECT nama FROM tbl_mesin WHERE jenis='$_GET[mc]' ORDER BY nama ASC"); 
		while($r=mysqli_fetch_array($qry1)){
		?>
    ...
    <tr>
      ...
      <td colspan="2"><select name="no_mesin" id="no_mesin" onchange="myFunction();" required="required">
        <option value="">Pilih</option>
        <?php $qry1=mysqli_query($con,"SELECT no_mesin FROM tbl_no_mesin WHERE jenis='$_GET[mc]' ORDER BY no_mesin ASC"); 
		while($r=mysqli_fetch_array($qry1)){
		?>
...
<?php } ?></td>
 ...
 <table width="100%" border="0">
	 ...
	 <?php if($_GET['mc']=="stenter" or $_GET['mc']=="oven"){ ?>
     <tr>
       <td scope="row"><h4>OMT</h4></td>
       <td>:</td>
       <td><input name="omt" type="text" id="omt" value="<?php echo $rw['omt'];?>" size="5" />
        <strong>&deg;</strong></td>
       <td scope="row"><?php if($_GET['mc']=="stenter"){ ?><h4>VMT</h4> <?php }?></td>
       <td>:</td>
       <td><?php if($_GET['mc']=="stenter"){ ?><input name="vmt" type="text" required id="vmt" value="<?php echo $rw['vmt'];?>" size="5"/>
         <strong>&deg;
         X</strong>
         <input name="vmt_time" type="text" required id="vmt_time" value="<?php echo $rw['t_vmt'];?>" size="5"/>
         <strong>second</strong><?php }?></td>
     </tr>
	 <?php } ?>
     <tr>
